# Demo of NextJS rewrite to bypass CORS in frontend

see the page `apps/web-app/next.config.js` for rewrite config
see the apollo config here - `apps/web-app/pages/_app.tsx`


# Document field Demo

this is demo for using Document field outside of keystone admin ui

1. clone
2. yarn install
3. run `yarn dev:api` in terminal
4. run `yarn dev:web` in a separate terminal
5. go to http://localhost:8100 and create a post after init (first user experience)

go to http://localhost:8101 to see it in action.


------
# Monorepo Starter based on ThinkMill's starter
